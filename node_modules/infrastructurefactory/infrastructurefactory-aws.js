...
module.exports = 
{
	...
	init: function (param)
	{
		...
		*/
		...
		entityos.add(
		{
			name: 'app-process-aws-ec2-instance-image-share-full',
			notes: 'Share an AMI and its underlying EBS snapshots with another AWS account',
			code: function ()
			{
				const event = entityos.get({scope: '_event'});
				const imageID = _.get(event, 'image.id');
				...
				{
					const infrastructureConfig = entityos.invoke('util-aws-get-config');
					const { EC2Client, DescribeImagesCommand, ModifyImageAttributeCommand, ModifySnapshotAttributeCommand } = require("@aws-sdk/client-ec2");
					const ec2 = new EC2Client(infrastructureConfig);

					// Step 1: Get AMI details to find snapshot IDs
					const describeCommand = new DescribeImagesCommand({ ImageIds: [imageID] });

					ec2.send(describeCommand).then(function(response)
					{
						const image = _.get(response, 'Images[0]');

						if (image == undefined)
						{
							entityos.invoke('util-end', 'AMI not found', '404');
							return;
						}

						const snapshotIds = [];

						_.each(image.BlockDeviceMappings, function(mapping)
						{
						...
						console.log('Snapshots to share:', snapshotIds);

						// Step 2: Share each snapshot
						const launchPermissions = targetAccountIDs.map(function(accountId)
						{
							return { UserId: accountId };
						});

						const snapshotPromises = snapshotIds.map(function(snapshotId)
						{
							const snapshotCommand = new ModifySnapshotAttributeCommand({
								SnapshotId: snapshotId,
							...
							return ec2.send(snapshotCommand);
						});

						// Step 3: Share the AMI itself
						const amiCommand = new ModifyImageAttributeCommand(
						{
							ImageId: imageID,
							LaunchPermission:
							{
						...
						const allPromises = [...snapshotPromises, ec2.send(amiCommand)];

						Promise.all(allPromises).then(function(results)
						{
							entityos.invoke('util-end',
							{
								message: 'AMI and snapshots shared successfully',
								imageId: imageID,
								snapshotsShared: snapshotIds,
								sharedWith: targetAccountIDs
							}, '200');
						})
						.catch(function(error)
						{
							entityos.invoke('util-end', 'Error sharing AMI/snapshots: ' + error.message, '500');
						});
					})
					.catch(function(error)
					{
						entityos.invoke('util-end', 'Error describing AMI: ' + error.message, '500');
					});
				}
			}
		});
